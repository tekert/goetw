...
var mof{{.Name}} = &etw.MofClassDef{
    ...
    Properties: []etw.MofPropertyDef{
        {{- range .Properties}}
        {ID: {{.ID}}, Name: "{{.Name}}", InType: etw.{{.InType}}
            {{- if .OutType}}, OutType: etw.{{.OutType}}{{- end -}}
...
var MofClassMapping2 = map[uint32]etw.MofKernelNames{
    ...
    guidToUint("{{$class.GUID}}"): {Name: "{{$class.Name}}", BaseId: etw.calcBaseId({{$i}})},
    {{- end}}
}
